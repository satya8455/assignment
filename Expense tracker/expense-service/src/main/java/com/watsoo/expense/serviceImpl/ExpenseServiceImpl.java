...
public class ExpenseServiceImpl implements ExpenseService {
	...
	public Response<?> createExpense(ExpenseDto expenseDto) {
		try {
			Response<UserDto> userResponse = walletFeignClient.getUserById(expenseDto.getUserId());
			...
			String month = DateUtil.formatDateToYearMonth(new Date());
			...
			Double totalSpentThisMonth = expenseRepository.sumOfExpenses(startDate, endDate, category.getId(),
	...
	public Response<?> getAllExpenses(Long userId, Long walletId, String monthString) {
		try {
			...
			Date[] monthRange = DateUtil.getMonthRange(monthString);
			Date startDate = DateUtil.removeTime(monthRange[0]);
			Date endDate = DateUtil.removeTime(monthRange[1]);
