...
public class WalletServiceImpl implements WalletService {
	...
	public Response<?> getAllWallet(Long userId) {
		try {
			...
			List<WalletDto> walletDtos = wallets.stream().map(Wallet::convertToDto).collect(Collectors.toList());

			return new Response<>(HttpStatus.OK.value(), "All Wallets fetched successfully", walletDtos);
		} catch (Exception e) {
	...
	public Response<?> createWalletType(WalletTypeDto walletTypeDto) {
		try {
			if (walletTypeDto.getUserId() == null) {
				return new Response<>(HttpStatus.BAD_REQUEST.value(), "Provide user-id", null);
			}
			Optional<User> loggedUser = userRepository.findById(walletTypeDto.getUserId());
			if (!loggedUser.isPresent()) {
				return new Response<>(HttpStatus.BAD_REQUEST.value(), "Provide valid userId", null);
			}
			...
			List<WalletType> walletTypes = walletTypeRepository.findAllAndfindByUserId(loggedUser.get().getId());
			boolean containsWallet = walletTypes.stream().anyMatch(e -> e.getName().equals(walletTypeDto.getName()));
			if (containsWallet) {
				return new Response<>(HttpStatus.OK.value(), "wallet type already exists", null);
			}
	...
	public Response<?> getAllWalletType(Long userId) {
		try {
			...
			List<WalletTypeDto> walllTypeDtos = walletTypes.stream().map(WalletType::convertToDto)
	...
	public Response<?> deductFromWallet(WalletDto walletDto) {
		try {
			...
			Optional<Wallet> walletOptional = walletRepository.findByIdAndUser_Id(walletDto.getId(),
					walletDto.getUserDto().getId());
